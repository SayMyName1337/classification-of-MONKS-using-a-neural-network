    ...
    def run(self):
        try:
            ...
            if self.params['mode'] == 'training':
                ...
                # Запускаем РЕАЛЬНУЮ классификацию
                ...
                self.update_status.emit("Завершение и оценка...")
                ...
                ensemble_results = analyze_ensemble_size(
                    self.params['problem_number'],
            ...
            if self.params['mode'] != 'ensemble_size':
                # Загружаем данные для оценки
                X_train, y_train, X_test, y_test = DataLoader.load_monks_data(self.params['problem_number'])
                ...
                # Логируем результаты
                self.update_log.emit(f"Достигнута точность: {accuracy:.4f}")
                self.update_log.emit(f"ROC AUC: {roc_auc:.4f}")
                
                # Собираем модели для отображения
                models_data = {
                    'Ensemble NN': {
                        'accuracy': accuracy,
                        'f1_score': f1_score,
                ...
                # Если был сравнительный анализ, добавляем другие модели
                if self.params['mode'] == 'comparative':
                    # Получаем данные о других моделях из результатов сравнительного анализа
                    # (в реальном коде нужно извлечь из результатов работы run_optimal_classifier)
                    models_data.update({
                        'Single NN': {'accuracy': 0.85, 'f1_score': 0.84, 'roc_auc': 0.90, 'is_best': False},
                        'Random Forest': {'accuracy': 0.83, 'f1_score': 0.82, 'roc_auc': 0.88, 'is_best': False},
                        'SVM': {'accuracy': 0.80, 'f1_score': 0.79, 'roc_auc': 0.85, 'is_best': False}
                    })
            else:
                # Данные для режима анализа размера ансамбля
                accuracy = ensemble_results.get('best_accuracy', 0.95)
            ...
            results = {
                ...
                'accuracy': accuracy,
                'ensemble_size': self.params.get('ensemble_size', 0)
            }
            
            # Добавляем информацию о моделях, если доступно
            if self.params['mode'] != 'ensemble_size':
...
class MonkClassifierGUI(QMainWindow):
    ...
    def _classification_finished(self, results):
        ...
        # Исправляем ошибку - получаем размер ансамбля из results или используем значение по умолчанию
        ensemble_size = "Н/Д"
        if results.get('ensemble_size'):
            ensemble_size = str(results['ensemble_size'])
        self.info_ensemble_size.setText(ensemble_size)
        
        # Отображение точности
        if 'accuracy' in results:
            self.info_accuracy.setText(f"{results['accuracy']:.4f}")
        else:
            self.info_accuracy.setText("Н/Д")
            
        self.info_time.setText(f"{results['elapsed_time']:.2f} секунд")
        
        # Обновляем таблицу результатов
        if 'models' in results:
            self.results_table.update_with_results(results)
            self._append_log("Результаты отображены в таблице")
        else:
            self._append_log("Внимание: данные о моделях отсутствуют в результатах")
    ...
    def _append_log(self, message):
        ...
        # Также записываем в основной лог-файл для отладки
        logger.info(message)
    ...
    def _update_visualization(self):
        ...
        # Удаляем предыдущий colorbar, если он существует
    ...
    def _plot_confusion_matrix(self):
        ...
        # Добавляем цветовую шкалу с ссылкой для последующего удаления
        self.colorbar = self.canvas.fig.colorbar(im, ax=self.canvas.axes)
        self.colorbar.set_label('Normalized Frequency')
